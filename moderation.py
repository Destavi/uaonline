...
class Moderation(commands.Cog):
    ...
    async def ban(self, interaction: discord.Interaction, member: discord.Member, duration: str = None, reason: str = "Не вказана"):
        ...
        try:
            ...
            log_mod_action(interaction.guild.id, "ban", interaction.user, member, f"{duration or 'perm'}: {reason}")
    ...
    @app_commands.command(name="mute", description="Видати таймаут (мут)")
    ...
    @app_commands.command(name="warn", description="Видати варн")
    async def warn(self, interaction: discord.Interaction, member: discord.Member, reason: str):
        ...
        await interaction.response.defer()
        conn = get_conn(); cur = conn.cursor()
        cur.execute("INSERT INTO warnings (guild_id, user_id, reason, admin_name, timestamp) VALUES (%s, %s, %s, %s, %s)", 
                   (str(interaction.guild.id), member.id, reason, interaction.user.display_name, datetime.now()))
        conn.commit()
    ...
    async def unban(self, interaction: discord.Interaction, user_id: str, reason: str = "Не вказана"):
        ...
        try:
            ...
            await interaction.guild.unban(user, reason=f"Знято: {interaction.user.display_name}")
            update_stat(interaction.guild.id, "ban_removed")
            remove_temp_ban(interaction.guild.id, user_id)
            await interaction.response.send_message(f"✅ {user.name} розбанений.")
        except Exception as e: await interaction.response.send_message(f"❌ Помилка: {e}")

    @app_commands.command(name="unmute", description="Зняти мут")
