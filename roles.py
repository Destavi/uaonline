    ...
    async def remove_faction_roles(self, interaction: discord.Interaction, member: discord.Member):
        ...
        if removed_roles:
            ...
            for role_name in removed_roles:
                ...
                update_stat(guild_id=interaction.guild.id, action_type="roles_removed", moderator_id=interaction.user.id)
...
class RoleApprovalView(discord.ui.View):
    ...
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            print(f"⚠️ Помилка витягування даних з ембеду: {e}")
            await interaction.followup.send("❌ Не вдалося витягти дані із запиту. Формат повідомлення було змінено.", ephemeral=True)
            return
        ...
        if not role:
            await interaction.followup.send(f"❌ Роль '{role_name}' не знайдена на сервері. Створіть її з точною назвою.", ephemeral=True)
            return
        ...
        if member:
            try:
                ...
                try:
                    update_stat(guild_id=interaction.guild.id, action_type="roles_issued", moderator_id=interaction.user.id)
                    log_mod_action(guild_id=interaction.guild.id, action_type="role_issued", admin=interaction.user, target=member, reason=f"Роль: {role_name}")
                except Exception as e_stat:
                    print(f"⚠️ Помилка оновлення статистики: {e_stat}")
                ...
                res_val = f"Схвалено адміністратором {interaction.user.mention}"
                if rank and nickname:
                    res_val += f"\nНікнейм змінено на: `{new_nick}`"
                
                embed.add_field(name="🏁 Результат", value=res_val, inline=False)
                await interaction.message.edit(embed=embed, view=None)
                
                await interaction.followup.send(f"✅ Роль {role_name} видана користувачу {member.mention}.", ephemeral=True)
            except discord.Forbidden:
                await interaction.followup.send(f"❌ Помилка: У бота недостатньо прав для видачі ролі '{role_name}'. Перевірте ієрархію ролей.", ephemeral=True)
            except Exception as e:
                print(f"❌ Помилка у approve: {e}")
                await interaction.followup.send(f"❌ Не вдалося видати роль: {e}", ephemeral=True)
        else:
            await interaction.followup.send("❌ Користувач не знайдений на сервері.", ephemeral=True)
